...
def is_table_empty(table, host='localhost', user='root', password='', database='ecogreendb'):
...
def read_mysql_table(table, host='localhost', user='root', password='', database='ecogreendb'):
    # Establish a connection to the MySQL database
    connection = pymysql.connect(
        host=host,
        user=user,
...
if not is_table_empty(table_name):
    ...
    #slang word
    path_dataslang = open("Data/kamus kata baku.csv")
    ...
    load_model = pickle.load(open('Model/FinalModelSVM.pkl','rb'))
    ...
    # Display unique labels in the results
    ...
    def delete_all_data_from_table(table, host='localhost', user='root', password='', database='ecogreendb'):
        # Establish a connection to the MySQL database
        connection = pymysql.connect(
            host=host,
            user=user,
    ...
    def insert_df_into_hasil_model(df, host='localhost', user='root', password='', database='ecogreendb'):
        # Establish a connection to the MySQL database
        connection = pymysql.connect(
            host=host,
            user=user,
...
st.title('Dashboard Analisis Sentimen Ecogreen')
...
with col2:
    st.metric(label='Netral (0)', value=jumlah_netral)
...
fig, ax = plt.subplots()
labels = ['Positif (1)', 'Netral (0)', 'Negatif (-1)']
jumlah_data = [jumlah_positif, jumlah_netral , jumlah_negatif]
